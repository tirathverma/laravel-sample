@extends('layouts.app')
@section('title', 'Customer Past Transactions')
@section('content')
<!-- START CONTENT -->
                    ...
                    <tr id="results">
                        ...
                        <td>
                            ...
                            foreach ($value->invoiceItem as $item) {
                                ...
                                $sale_price += $item->HKD_price * $item->quantity;

                                $child_data .= '<tr><td>' . $no . '</td><td>' . $item->stock['name'] . '</td><td>' . $item->stock['vintage'] . '</td><td>' . $item->stock['size'] . '</td><td>' . $item->stock['owc'] . '</td><td>' . $item->quantity . '</td><td>' . '$' . number_format($item->cost_hkd, 2, '.', '') . '</td><td>' . '$' . number_format($item->HKD_price, 2, '.', '') . '</td><td>' . '$' . number_format($item->HKD_price * $item->quantity, 2, '.', '') . '</td></tr>';
                            }
